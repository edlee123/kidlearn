...
class RiaritHssbg(HierarchicalSSBG):
    ...
    # Accesser
    def get_KC(self):
        return self.SSBGs[self.main_act].competencies
    ...
    def compute_act_lvl(self, act, RT=None, **kwargs):
        lvl = {}
        lvl[self.main_act] = [1] * self.ncompetencies
        self.ssbg_used = []
    ...
    def load_Error(self, RT_ID="MAIN", directory='assets/algorithm/error_RT/'):
        ...
        try:
            ...
            for lin in lines[2:]:
                ...
                self.error_tab.append(aux[0:self.ncompetencies])
        except:
            return
        return
    ...
    def return_answer_impact(self, result, error_ID=None):
        if result == 1:
            return [1] * max(1, self.ncompetencies)
        elif error_ID != None:
            error_num = self.error_ID_tab.index(error_ID)
        else:
            return [0] * max(1, self.ncompetencies)
...
class RiaritSsbg(SSBanditGroup):
    ...
    def instanciate_ssb(self, ii, is_hierarchical):
        ...
        return RiaritSsb(ii, self.nvalue[ii], self.ncompetencies, self.requer[ii], self.stop[ii], is_hierarchical=is_hierarchical, param_values=self.param_values[ii], params=params)

    def get_estim_level(self, **kwargs):
        if "dict_form" in kwargs.keys():
            return {key: value for (key, value) in zip(self.competencies, self.estim_level)}
    ...
    def calDiffLvl(self, lvl):
        lvlDiff = []
        for i in range(self.ncompetencies):
    ...
    def load_jsonRT(self, RT):
        ...
        self.competencies = params_RT["competencies"]
        self.ncompetencies = len(self.competencies)
        self.estim_level = [0] * self.ncompetencies
        ...
        for num_act in range(self.nactions):
            for key, val in params_RT["table"][self.actions[num_act]].items():
                ...
                self.requer[num_act].append(func.fill_data(val["requir"], self.ncompetencies))
                self.stop[num_act].append(func.fill_data(val["deacti"], self.ncompetencies))
    ...
    def load_textRT(self, RT):
        ...
        self.competencies = tmp[1:len(tmp)]
        self.ncompetencies = len(self.competencies)
        self.estim_level = [0] * self.ncompetencies
        ...
        for lin in lines[3:]:
            ...
            self.RT[int(tmp[0]) - 1].append(aux[0:self.ncompetencies])
            self.requer[int(tmp[0]) - 1].append(aux[self.ncompetencies:2 * self.ncompetencies])
            self.stop[int(tmp[0]) - 1].append(func.fill_data(aux[2 * self.ncompetencies:], self.ncompetencies))
        self.nvalue.append(nval)
        reader.close()
    ...
    def RTable(self, act, lvl=None, **kwargs):
        ...
        # if "dict_form" in kwargs.keys():
        #    lvl_dict = {key: value for (key,value) in zip(self.competencies,lvl)}
        #    return lvl_dict

        return lvl

    def calcul_reward(self, lvl, result, answer_impact):
        r_KC = [0] * self.ncompetencies
        for i in range(self.ncompetencies):
        ...
        r = max(0, sum(r_KC) / self.ncompetencies)
        return r
