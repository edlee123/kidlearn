...
# The Kidlearn library includes a set of algorithms designed to guide a pedagogical sequence of activities,
# these algorithms are implemented in the seq_manager module of the library (explained in another notebook).
# To define the different possible exercises, we use "R-Table" which are described in our articles.
# To test and use these algorithms, we defined student models, implemented in student module, which
#  have some skills to learn through a didactic sequence managed by an algorithm.

# In this notebook, we will focus on the experimentation module, which permit to manage different level of simulation.
# The parameter files are located in the folder paramas_files/, and the using R-Tables in RT/.

import kidlearn_lib as k_lib
import plot_graphics as graph # fixed this, used to be graph_

##The experimentation objects

# We will introduce the different objects involved in experimentation module.
# In this notebook we will only use the "Q" student model and the "N1" R-Table.

##Working session

# A Working session is defined by a student and a sequence manager.
# It will permit to manage the activity sequence for the student s with the algorithm A.
...
# The goal of the Experiment object is to give a tool to follow and analyze data from a population of students using
# different sequences managers, for example to compare RiARiT, ZPDES with a random or predefined sequence.
